...
enum { INSERT_KEY1 = 1, INSERT_KEY2, EXIT_KEY1, EXIT_KEY2};
uint8_t KeyStatus;
...
const uint8_t resCmd32[4][8] = {
  {0x01, 0x00, 0x30, 0x79, 0x39, 0x71, 0x44, 0x00},
...
void ESL_EvaluateCommand(void) {
  ...
  switch (SerialRxMSG.frame.Id) {
		...
		case 0xF5:
//			if ((SerialRxMSG.frame.DataBuffer[0] == 0x02) || (SerialRxMSG.frame.DataBuffer[0] == 0x22) || (SerialRxMSG.frame.DataBuffer[0] == 0x41)) {
//				IndexCmd32 = 0;
//				IndexCmd37 = 0;
  ...
  switch (id) {
    ...
    case 0xC1: // LIN Read Command
      ...
      if (KeyStatus == INSERT_KEY1) {
        ...
        if (LockPos.Lo == 0x01) LockPos.Lo = 0x04;
        else if (++LockPos.Lo == 0x016) LockPos.Hi = 0x07;
      }
        ...
        if (LockPos.Lo == 0x01) LockPos.Lo = 0x06;
        else if (++LockPos.Lo == 0x012) LockPos.Hi = 0x04;
      }
