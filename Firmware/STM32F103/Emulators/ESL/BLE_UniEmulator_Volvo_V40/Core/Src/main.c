...
void LED_Red_StatusChange_Callback(bool status)
{
    if (TIM3->CCR1 == 250)
    {
...
void LED_Green_StatusChange_Callback(bool status)
{
}

void LED_Blue_StatusChange_Callback(bool status)
{
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    if (EEPROM_Init(AT24C32) != HAL_OK)
    {
		...
		// When Bootloader starts SELF_TIMED_READ_CYCLE time has elapsed
		eepromTimer = SELF_TIMED_READ_CYCLE;

    SmartLED_Init(LED_R_GPIO_Port, LED_R_Pin, true, &LED_Red_StatusChange_Callback, NULL);
    SmartLED_Init(LED_G_GPIO_Port, LED_G_Pin, true, &LED_Green_StatusChange_Callback, NULL);
    SmartLED_Init(LED_B_GPIO_Port, LED_B_Pin, true, &LED_Blue_StatusChange_Callback, NULL);
		
		// Smart LED test
		//SmartLED_SetBlinkDelayed(LED_R, 200, 3, 3000); 
		//SmartLED_SetBlinkDelayed(LED_G, 500, 5, 7000);
		//SmartLED_SetPatternDelayed(LED_B, "--... ", 5, 5000);
