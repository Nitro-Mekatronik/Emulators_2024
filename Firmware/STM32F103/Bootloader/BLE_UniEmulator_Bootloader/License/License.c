...
// void Update_Check_License(void) {
...
//        // Handle failure in reading and decrypting the unique ID from EEPROM
//				Error_Handler();
//        return;
//    }
//
...
// Clear the public key and IV when exiting license check
...
int read_and_decrypt_unique_id(uint8_t *decrypted_id, size_t len)
{
    ...
    uint8_t level = eeprom_encrypted_id[16] >> 4;
    uint8_t shift = eeprom_encrypted_id[16] & 0x0F;
...
//    uint8_t EEPROM_Unique[ENCRYPTED_ID_LENGTH + ENCRYPTED_ALGORITHM_LENGTH];  // Array to hold the encrypted unique ID
//    for EEPROM
//
